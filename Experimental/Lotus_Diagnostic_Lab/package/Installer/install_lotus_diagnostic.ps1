...
function Resolve-ComfyRoot {
  ...
  if ($Explicit) {
    ...
    if ((Test-Path (Join-Path $r 'main.py')) -and (Test-Path (Join-Path $r 'custom_nodes'))) { return $r }
  ...
  if (Test-Path $base) {
    Get-ChildItem -LiteralPath $base -Directory -ErrorAction SilentlyContinue | ForEach-Object {
      $p1 = Join-Path $_.FullName 'ComfyUI'
      $p2 = $_.FullName
      if ((Test-Path (Join-Path $p1 'main.py'))) { $script:candidates += $p1 }
      if ((Test-Path (Join-Path $p2 'main.py'))) { $script:candidates += $p2 }
    }
  }

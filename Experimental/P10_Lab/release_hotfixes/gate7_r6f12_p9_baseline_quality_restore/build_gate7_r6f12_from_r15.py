...
R6F="ConceptGhost_v1.54_P10_GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE"
...
def patch_entrypoints(root):
    ...
    replacements=(
        ("Gate 7 Preview r5 + Geometric Evidence + Run-Local Audit","Gate 7 R6F12 + P9 Baseline Quality Restore"),
        (R5,R6F),
        ("before_gate7_preview_r5_geometric_evidence_audit_fix","before_gate7_r6f12_p9_baseline_quality_restore"),
        ("P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_READY_VERIFY.json","P10_GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE_READY_VERIFY.json"),
        ("CONCEPTGHOST_P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS","CONCEPTGHOST_P10_GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE_RUNTIME_VERIFY_PASS"),
    )
    for old,new in replacements:
        install=install.replace(old,new)
        verify=verify.replace(old,new)
    wt(root/"Installer/install_gate7_r6f12_p9_baseline_quality_restore.ps1",install)
    wt(root/"Installer/verify_gate7_r6f12_p9_baseline_quality_restore.ps1",verify)
    old_install.unlink()
    old_verify.unlink()
...
cd /d "%~dp0"
echo ============================================================
echo 03 - ConceptGhost P10 Gate 7 R6F12
...
echo ============================================================
if not exist "%~dp0Installer\\install_gate7_r6f12_p9_baseline_quality_restore.ps1" (
  echo [FAIL] Required R6F installer entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\install_gate7_r6f12_p9_baseline_quality_restore.ps1"
if errorlevel 1 (
...
cd /d "%~dp0"
echo ============================================================
echo 04 - VERIFY ConceptGhost P10 Gate 7 R6F12
echo ============================================================
if not exist "%~dp0Installer\\verify_gate7_r6f12_p9_baseline_quality_restore.ps1" (
  echo [FAIL] Required R6F verifier entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\verify_gate7_r6f12_p9_baseline_quality_restore.ps1"
if errorlevel 1 (
...
def patch_production_workflow(root):
    ...
    workflow=json.loads(rt(path))
    extra=workflow.setdefault("extra",{}).setdefault("conceptghost",{})
    extra["gate7_preview_release"]="GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE"
...
def self_test() -> int:
    ...
    print("CONCEPTGHOST_FLEXGEMM_TRITON32_R6F12_SELFTEST_PASS")
    return 0
    ...
    if cc < (8,0):
        if _minor(torch.__version__) != (2,6) or _minor(triton_version) != (3,2):
            raise RuntimeError(
                ...
                "the private runtime torch 2.6.x + Triton 3.2.x. Re-run the R6F12 installer. "
...
'''
    ...
    lock["release"]="ConceptGhost v1.54 R6F12 MoGe3 Turing Runtime Fix"
    ...
    guide=root/"USER_GUIDE_GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE.md"
    wt(guide,rt(guide)+"""
## R6F12 MoGe-3 Turing runtime correction
...
R6F12 applies a bounded compatibility bridge to the ConceptGhost-private FlexGEMM hashmap kernels. FlexGEMM's Python wrappers already serialize keys and queries into padded torch.int32 tensors and pass D as the number of int32 words, so R6F12 removes Triton-JIT dtype-width introspection from those kernels and consumes that existing host-side packing contract directly. Model weights, resolution, refine steps and geometry authority are unchanged.
""")

    validation=root/"P10_GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE_VALIDATION.txt"
    wt(validation,rt(validation)+"""
R6F12 MoGe-3 Turing runtime compatibility:
...
""")
    ...
    test=root/"Installer/test_r6f12_p9_baseline_quality_restore.py"
...
def main(root):
    ...
    for token in ("SANITIZE_NON_CONSTEXPR_TRITON_ANNOTATIONS","sanitize_triton32_annotations","patch_reduce_or_compat","_conceptghost_or_combine","combine_fn=_conceptghost_or_combine","--self-test"):
        if token not in patch: errors.append("R6F12 annotation sanitizer missing "+token)
    try:
        compile(patch, str(patch_path), "exec")
    except SyntaxError as exc:
        errors.append("R6F12 bridge syntax error "+repr(exc))
    probe=subprocess.run([sys.executable,str(patch_path),"--self-test"],capture_output=True,text=True)
    if probe.returncode != 0 or "CONCEPTGHOST_FLEXGEMM_TRITON32_R6F12_SELFTEST_PASS" not in probe.stdout:
        errors.append("R6F12 bridge self-test failed: "+probe.stdout+" "+probe.stderr)
    ...
    print("CONCEPTGHOST_R6F12_P9_BASELINE_QUALITY_RESTORE_PASS"); return 0
...
def patch_p9_baseline_quality_restore(root):
    """Restore the frozen v0.36 PrimaryMesh topology rule.

    P9.10 later introduced a boundary-strength-dependent depth threshold into
    the shared PrimaryMesh builder. That changed the Baseline/P9 generation
    path even though the P9 contract required the stable v0.36 branch to keep
    its established generation behavior. R6F12 makes geometric boundary and
    normal evidence diagnostic again for this baseline meshing step while
    retaining the v0.36 fixed relative-depth threshold.
    """
    root=Path(root)
    nodes=root/"Payload/custom_nodes/ConceptGhost_Stage68/nodes.py"
    text=rt(nodes)

    old_filter='''        boundary_full = np.asarray(canonical.get("geometric_boundary_strength", []), dtype=np.float64).reshape(-1)
        if len(boundary_full) == len(xyz_full):
            face_boundary = np.max(np.clip(boundary_full[candidate_faces], 0.0, 1.0), axis=1)
            effective_rtol = np.maximum(rtol * 0.25, rtol * (1.0 - 0.60 * face_boundary))
        else:
            face_boundary = np.zeros(len(candidate_faces), dtype=np.float64)
            effective_rtol = np.full(len(candidate_faces), rtol, dtype=np.float64)
        forbidden = (~positive) | (~np.isfinite(max_jump)) | (max_jump >= effective_rtol)

        # P9.10: reliable upstream normal evidence participates in topology creation.
        # It never replaces topology-derived export normals; it only prevents a face
        # from bridging a strong, independently supported normal/boundary break.
        rejected_normal_boundary_faces = 0
        source_normals = np.asarray(canonical.get("normal_world", []), dtype=np.float64)
        source_normal_conf = np.asarray(canonical.get("normal_confidence", []), dtype=np.float64).reshape(-1)
        if source_normals.ndim == 2 and source_normals.shape == (len(xyz_full), 3) and len(source_normal_conf) == len(xyz_full):
            ns = source_normals[candidate_faces]
            nl = np.linalg.norm(ns, axis=2)
            nok = np.isfinite(nl).all(axis=1) & (nl > 1e-8).all(axis=1)
            nsu = np.zeros_like(ns)
            nsu[nok] = ns[nok] / nl[nok, :, None]
            d01 = np.clip(np.sum(nsu[:,0] * nsu[:,1], axis=1), -1.0, 1.0)
            d12 = np.clip(np.sum(nsu[:,1] * nsu[:,2], axis=1), -1.0, 1.0)
            d20 = np.clip(np.sum(nsu[:,2] * nsu[:,0], axis=1), -1.0, 1.0)
            min_dot = np.minimum(np.minimum(d01, d12), d20)
            max_angle = np.degrees(np.arccos(min_dot))
            face_nconf = np.min(np.clip(source_normal_conf[candidate_faces], 0.0, 1.0), axis=1)
            normal_bridge = nok & (face_nconf >= 0.55) & (face_boundary >= 0.35) & (max_angle >= 45.0)
            rejected_normal_boundary_faces = int(np.count_nonzero(normal_bridge & ~forbidden))
            forbidden |= normal_bridge
'''
    new_filter='''        # R6F12 QUALITY RESTORE:
        # The frozen v0.36 stable baseline used one fixed relative-depth
        # threshold. P9.10 later made that threshold as low as 25% of the
        # configured value using geometric_boundary_strength. That was a
        # topology-changing refinement leaking into Baseline/P9 and it
        # over-fragmented PrimaryMesh. Boundary/normal evidence remains
        # available downstream, but does not tighten this stable meshing gate.
        boundary_full = np.asarray(canonical.get("geometric_boundary_strength", []), dtype=np.float64).reshape(-1)
        if len(boundary_full) == len(xyz_full):
            face_boundary = np.max(np.clip(boundary_full[candidate_faces], 0.0, 1.0), axis=1)
        else:
            face_boundary = np.zeros(len(candidate_faces), dtype=np.float64)
...
'''
    if old_filter not in text:
        raise RuntimeError("R6F12 could not locate the P9.10 adaptive topology filter")
    text=text.replace(old_filter,new_filter,1)

    old_final='''        if len(boundary_full) == len(xyz_full):
            final_boundary = np.max(np.clip(boundary_full[accepted_global_faces], 0.0, 1.0), axis=1)
            final_rtol = np.maximum(float(depth_edge_rtol) * 0.25, float(depth_edge_rtol) * (1.0 - 0.60 * final_boundary))
        else:
            final_boundary = np.zeros(len(accepted_global_faces), dtype=np.float64)
            final_rtol = np.full(len(accepted_global_faces), float(depth_edge_rtol), dtype=np.float64)
        final_forbidden = (~final_positive) | (~np.isfinite(final_jump)) | (final_jump >= final_rtol)
        if source_normals.ndim == 2 and source_normals.shape == (len(xyz_full), 3) and len(source_normal_conf) == len(xyz_full):
            fns = source_normals[accepted_global_faces]
            fnl = np.linalg.norm(fns, axis=2)
            fnok = np.isfinite(fnl).all(axis=1) & (fnl > 1e-8).all(axis=1)
            fnu = np.zeros_like(fns); fnu[fnok] = fns[fnok] / fnl[fnok,:,None]
            fd01=np.clip(np.sum(fnu[:,0]*fnu[:,1],axis=1),-1.0,1.0); fd12=np.clip(np.sum(fnu[:,1]*fnu[:,2],axis=1),-1.0,1.0); fd20=np.clip(np.sum(fnu[:,2]*fnu[:,0],axis=1),-1.0,1.0)
            fangle=np.degrees(np.arccos(np.minimum(np.minimum(fd01,fd12),fd20)))
            fnconf=np.min(np.clip(source_normal_conf[accepted_global_faces],0.0,1.0),axis=1)
            final_forbidden |= fnok & (fnconf>=0.55) & (final_boundary>=0.35) & (fangle>=45.0)
'''
    new_final='''        # Independent proof must use the same frozen v0.36 fixed threshold
        # as the generation pass; diagnostic boundary evidence cannot tighten it.
        final_forbidden = (~final_positive) | (~np.isfinite(final_jump)) | (final_jump >= float(depth_edge_rtol))
'''
    if old_final not in text:
        raise RuntimeError("R6F12 could not locate the adaptive final topology proof")
    text=text.replace(old_final,new_final,1)

    old_report='''        "rejected_normal_boundary_faces": int(rejected_normal_boundary_faces),
        "p9_boundary_adaptive_depth_threshold": bool(high_fidelity and geometric_boundary_attr is not None),
'''
    new_report='''        "rejected_normal_boundary_faces": int(rejected_normal_boundary_faces),
...
'''
    if old_report not in text:
        raise RuntimeError("R6F12 could not locate P9 adaptive report fields")
...
def main(root):
    root=Path(root).resolve(); errors=[]
    nodes=root/"Payload/custom_nodes/ConceptGhost_Stage68/nodes.py"
    source=nodes.read_text(encoding="utf-8-sig")
    fn=_primary_mesh_function(source)

    required=(
        "V036_STABLE_FIXED_DEPTH_RTOL",
        "DIAGNOSTIC_ONLY_AT_PRIMARYMESH_GATE",
        "max_jump >= rtol",
        "final_jump >= float(depth_edge_rtol)",
        '"p9_boundary_adaptive_depth_threshold": False',
    )
    for token in required:
        if token not in fn: errors.append("PrimaryMesh restore missing "+token)

    forbidden=(
        "effective_rtol =",
        "final_rtol =",
        "forbidden |= normal_bridge",
        "final_forbidden |= fnok",
    )
    for token in forbidden:
        if token in fn: errors.append("adaptive topology mutation remains: "+token)

    # Exact regression fixture: v0.36 with rtol=.04 must keep 1.5% and 3.0%
    # depth jumps even under a strong diagnostic boundary; only 4.5% is cut.
    jumps=[0.015,0.030,0.045]
    stable=[x>=0.04 for x in jumps]
    leaked_adaptive=[x>=max(0.04*0.25,0.04*(1.0-0.60*1.0)) for x in jumps]
    if stable != [False,False,True]:
        errors.append("stable fixed-threshold fixture changed")
    if leaked_adaptive != [False,True,True]:
        errors.append("regression fixture no longer distinguishes adaptive policy")

    contracts=(root/"Payload/custom_nodes/ConceptGhost_Stage68/conceptghost_contracts.py").read_text(encoding="utf-8-sig")
    for token in ('"Ruicheng/moge-3-vitg"','"resolution_level": 9','"refine_steps": 7','"depth_edge_rtol": 0.04'):
        if token not in contracts: errors.append("High Fidelity contract changed: "+token)

    # Preserve all R6F11 runtime compatibility work; this hotfix is P9 topology-only.
    patch=root/"Runtime/MoGeRuntime/worker/patch_flexgemm_triton32.py"
    ptext=patch.read_text(encoding="utf-8-sig")
    for token in ("patch_reduce_or_compat","sanitize_triton32_annotations"):
        if token not in ptext: errors.append("R6F11 runtime regression: "+token)
...
def patch_contracts(root,source_commit,package_commit):
    ...
    text=rt(p).replace(
        ...
        "GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE_RUNTIME_VERIFY_PASS",
    ).replace(
        "Installer/install_gate7_preview_r5_geometric_evidence_audit_fix.ps1",
        "Installer/install_gate7_r6f12_p9_baseline_quality_restore.ps1",
    ).replace(
        "Installer/verify_gate7_preview_r5_geometric_evidence_audit_fix.ps1",
        "Installer/verify_gate7_r6f12_p9_baseline_quality_restore.ps1",
    )
    wt(p,text)
    ...
    wt(p,rt(p).replace(
        ...
        "USER_GUIDE_GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE.md",
    ))
...
"""
    wt(root/"USER_GUIDE_GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE.md",guide)
    old_guide.unlink()

    wt(root/"Payload/docs/40_R6F_ROUTE_EDITOR_FINAL.md",f"""# ConceptGhost P10 Gate 7 R6F12 — Route Editor Final
...
""")
    ...
    data.update({
        "schema":"ConceptGhost.P10Gate7R6F12CodeManifest.v0.1",
        "release":R6F,
    ...
    for name in ("RELEASE.json","P10_DR9_RELEASE.json"):
        ...
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F12Release.v0.1",
            "release":R6F,
            "source_commit":source_commit,
    ...
    wt(root/"P10_GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE_VALIDATION.txt",f"""ConceptGhost P10 Gate 7 R6F12 Route Editor Final
...
def main(root):
 ...
 if errors:
  print('\\n'.join('[FAIL] '+x for x in errors)); return 1
 print('CONCEPTGHOST_GATE7_R6F12_P9_BASELINE_QUALITY_RESTORE_PASS'); return 0
...
def rebuild(root,source_commit):
    ...
    for name in ("BUNDLE_MANIFEST.json","P10_DR9_BUNDLE_MANIFEST.json"):
        ...
        data=json.loads(rt(path))
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F12Bundle.v0.1",
            "release":R6F,
            "source_commit":source_commit,
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="cg-r6f-") as temp:
        ...
        patch_p9_baseline_quality_restore(root)

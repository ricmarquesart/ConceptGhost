...
R6F="ConceptGhost_v1.54_P10_GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX"
...
def patch_entrypoints(root):
    ...
    replacements=(
        ("Gate 7 Preview r5 + Geometric Evidence + Run-Local Audit","Gate 7 R6F22 + MoGe-3 Turing Runtime Fix"),
        (R5,R6F),
        ("before_gate7_preview_r5_geometric_evidence_audit_fix","before_gate7_r6f2_moge3_turing_runtime_fix"),
        ("P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_READY_VERIFY.json","P10_GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX_READY_VERIFY.json"),
        ("CONCEPTGHOST_P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS","CONCEPTGHOST_P10_GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX_RUNTIME_VERIFY_PASS"),
    )
    for old,new in replacements:
        install=install.replace(old,new)
        verify=verify.replace(old,new)
    wt(root/"Installer/install_gate7_r6f2_moge3_turing_runtime_fix.ps1",install)
    wt(root/"Installer/verify_gate7_r6f2_moge3_turing_runtime_fix.ps1",verify)
    old_install.unlink()
    old_verify.unlink()
...
cd /d "%~dp0"
echo ============================================================
echo 03 - ConceptGhost P10 Gate 7 R6F2
...
echo ============================================================
if not exist "%~dp0Installer\\install_gate7_r6f2_moge3_turing_runtime_fix.ps1" (
  echo [FAIL] Required R6F installer entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\install_gate7_r6f2_moge3_turing_runtime_fix.ps1"
if errorlevel 1 (
...
cd /d "%~dp0"
echo ============================================================
echo 04 - VERIFY ConceptGhost P10 Gate 7 R6F2
echo ============================================================
if not exist "%~dp0Installer\\verify_gate7_r6f2_moge3_turing_runtime_fix.ps1" (
  echo [FAIL] Required R6F verifier entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\verify_gate7_r6f2_moge3_turing_runtime_fix.ps1"
if errorlevel 1 (
...
def patch_production_workflow(root):
    ...
    workflow=json.loads(rt(path))
    extra=workflow.setdefault("extra",{}).setdefault("conceptghost",{})
    extra["gate7_preview_release"]="GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX"
    ...
    if cc < (8,0):
        if _minor(torch.__version__) != (2,6) or _minor(triton_version) != (3,2):
            raise RuntimeError(
                ...
                "the private runtime torch 2.6.x + Triton 3.2.x. Re-run the R6F2 installer. "
...
'''
    ...
    lock["release"]="ConceptGhost v1.54 R6F2 MoGe3 Turing Runtime Fix"
    ...
    guide=root/"USER_GUIDE_GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX.md"
    wt(guide,rt(guide)+"""
## R6F2 MoGe-3 Turing runtime correction
...
""")
    ...
    validation=root/"P10_GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX_VALIDATION.txt"
    wt(validation,rt(validation)+"""
R6F2 MoGe-3 Turing runtime compatibility:
...
def main(root):
    ...
    for token in ("cu124","torch==2.6.0","torchvision==0.21.0","triton-windows>=3.2,<3.3","verify_vitg_runtime.py"):
    ...
    print("CONCEPTGHOST_R6F2_MOGE3_TURING_RUNTIME_FIX_PASS"); return 0
...
def patch_contracts(root,source_commit,package_commit):
    ...
    wt(p,rt(p).replace(
        ...
        "USER_GUIDE_GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX.md",
    ))
    ...
    guide=rt(old_guide).replace(
        ...
        "Gate 7 R6F22 + MoGe-3 Turing Runtime Fix",
    )
    guide += """
...
"""
    wt(root/"USER_GUIDE_GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX.md",guide)
    old_guide.unlink()

    wt(root/"Payload/docs/40_R6F_ROUTE_EDITOR_FINAL.md",f"""# ConceptGhost P10 Gate 7 R6F2 — Route Editor Final
...
""")
    ...
    data.update({
        "schema":"ConceptGhost.P10Gate7R6F2CodeManifest.v0.1",
        "release":R6F,
    ...
    for name in ("RELEASE.json","P10_DR9_RELEASE.json"):
        ...
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F2Release.v0.1",
            "release":R6F,
            "source_commit":source_commit,
    ...
    wt(root/"P10_GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX_VALIDATION.txt",f"""ConceptGhost P10 Gate 7 R6F2 Route Editor Final
...
def main(root):
 ...
 print('CONCEPTGHOST_GATE7_R6F2_MOGE3_TURING_RUNTIME_FIX_PASS'); return 0
...
def rebuild(root,source_commit):
    ...
    for name in ("BUNDLE_MANIFEST.json","P10_DR9_BUNDLE_MANIFEST.json"):
        ...
        data=json.loads(rt(path))
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F2Bundle.v0.1",
            "release":R6F,
            "source_commit":source_commit,

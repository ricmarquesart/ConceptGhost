...
R6F="ConceptGhost_v1.54_P10_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX"
...
def patch_entrypoints(root):
    ...
    replacements=(
        ("Gate 7 Preview r5 + Geometric Evidence + Run-Local Audit","Gate 7 R6F3 + FlexGEMM/Triton 3.2 Compatibility Fix"),
        (R5,R6F),
        ("before_gate7_preview_r5_geometric_evidence_audit_fix","before_gate7_r6f3_flexgemm_triton32_compat_fix"),
        ("P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_READY_VERIFY.json","P10_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX_READY_VERIFY.json"),
        ("CONCEPTGHOST_P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS","CONCEPTGHOST_P10_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX_RUNTIME_VERIFY_PASS"),
    )
    for old,new in replacements:
        install=install.replace(old,new)
        verify=verify.replace(old,new)
    wt(root/"Installer/install_gate7_r6f3_flexgemm_triton32_compat_fix.ps1",install)
    wt(root/"Installer/verify_gate7_r6f3_flexgemm_triton32_compat_fix.ps1",verify)
    old_install.unlink()
    old_verify.unlink()
...
cd /d "%~dp0"
echo ============================================================
echo 03 - ConceptGhost P10 Gate 7 R6F3
...
echo ============================================================
if not exist "%~dp0Installer\\install_gate7_r6f3_flexgemm_triton32_compat_fix.ps1" (
  echo [FAIL] Required R6F installer entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\install_gate7_r6f3_flexgemm_triton32_compat_fix.ps1"
if errorlevel 1 (
...
cd /d "%~dp0"
echo ============================================================
echo 04 - VERIFY ConceptGhost P10 Gate 7 R6F3
echo ============================================================
if not exist "%~dp0Installer\\verify_gate7_r6f3_flexgemm_triton32_compat_fix.ps1" (
  echo [FAIL] Required R6F verifier entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\verify_gate7_r6f3_flexgemm_triton32_compat_fix.ps1"
if errorlevel 1 (
...
def patch_production_workflow(root):
    ...
    workflow=json.loads(rt(path))
    extra=workflow.setdefault("extra",{}).setdefault("conceptghost",{})
    extra["gate7_preview_release"]="GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX"
...
def main() -> int:
    ...
    report = {
        "schema": "ConceptGhost.FlexGEMMTriton32Compatibility.v0.1",
        "status": "FAIL",
        "runtime_root": str(runtime_root),
        "scope": "CONCEPTGHOST_PRIVATE_MOGE_RUNTIME_ONLY",
        "upstream_flexgemm_commit": "b2fadb29d41846c7981ade6801ffc689fae119cf",
        "reason": "FlexGEMM pinned by MoGe-3 uses Triton dtype.itemsize API introduced after Triton 3.2; Turing/sm75 requires Triton 3.2.x.",
    }
    ...
    replacements = (
        ("query_vec.dtype.itemsize", "(query_vec.dtype.primitive_bitwidth // 8)", 1),
        ("keys_ptr.dtype.element_ty.itemsize", "(keys_ptr.dtype.element_ty.primitive_bitwidth // 8)", 4),
        ("vec.dtype.itemsize", "(vec.dtype.primitive_bitwidth // 8)", 6),
    )
    patched = original
    counts = {}
    ...
    for old, new, expected in replacements:
        ...
        counts[old] = {"old_count": old_count, "new_count": new_count, "expected_replacements": expected}
        if old_count:
            already_patched = False
            if old_count != expected:
                report["error"] = f"Unexpected FlexGEMM source shape for {old}: {old_count} != {expected}"
            ...
            report["error"] = f"Neither expected legacy nor patched token count found for {old}"
            report["counts"] = counts
            report_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
            print(json.dumps(report, indent=2))
            return 5

    if ".dtype.itemsize" in patched or ".dtype.element_ty.itemsize" in patched:
        report["error"] = "Unsupported Triton dtype.itemsize token remains after patch"
        report_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
        print(json.dumps(report, indent=2))
        return 6
    ...
    if cc < (8,0):
        if _minor(torch.__version__) != (2,6) or _minor(triton_version) != (3,2):
            raise RuntimeError(
                ...
                "the private runtime torch 2.6.x + Triton 3.2.x. Re-run the R6F3 installer. "
...
'''
    ...
    lock["runtime_compatibility"]={
        ...
        "flexgemm_triton32_bridge":"dtype.itemsize -> primitive_bitwidth/8 inside private runtime only",
        "geometry_contract_changed":False,
    }
    lock["release"]="ConceptGhost v1.54 R6F3 MoGe3 Turing Runtime Fix"
    ...
    guide=root/"USER_GUIDE_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX.md"
    wt(guide,rt(guide)+"""
## R6F3 MoGe-3 Turing runtime correction
...
R6F3 also applies a bounded compatibility bridge to the ConceptGhost-private FlexGEMM hashmap kernels: Triton 3.2 exposes primitive_bitwidth rather than the newer dtype.itemsize property used by MoGe-3's pinned FlexGEMM commit. The bridge changes only compile-time dtype-width introspection; model weights, resolution, refine steps and geometry authority are unchanged.
""")

    validation=root/"P10_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX_VALIDATION.txt"
    wt(validation,rt(validation)+"""
R6F3 MoGe-3 Turing runtime compatibility:
...
- FlexGEMM/Triton 3.2 dtype-width compatibility bridge: INCLUDED
...
def main(root):
    ...
    for token in ("primitive_bitwidth // 8","expected_replacements","FLEXGEMM_TRITON32_PATCH.json","geometry_contract_changed"):
    ...
    print("CONCEPTGHOST_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX_PASS"); return 0
...
def patch_contracts(root,source_commit,package_commit):
    ...
    wt(p,rt(p).replace(
        ...
        "CONCEPTGHOST_P10_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX_RUNTIME_VERIFY_PASS",
    ))

    p=root/"Installer/test_gate7_preview_bundle.py"
    text=rt(p).replace(
        "GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS",
        "GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX_RUNTIME_VERIFY_PASS",
    ).replace(
    ...
    wt(p,rt(p).replace(
        ...
        "USER_GUIDE_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX.md",
    ))
...
"""
    wt(root/"USER_GUIDE_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX.md",guide)
    old_guide.unlink()

    wt(root/"Payload/docs/40_R6F_ROUTE_EDITOR_FINAL.md",f"""# ConceptGhost P10 Gate 7 R6F3 — Route Editor Final
...
""")
    ...
    data.update({
        "schema":"ConceptGhost.P10Gate7R6F3CodeManifest.v0.1",
        "release":R6F,
    ...
    for name in ("RELEASE.json","P10_DR9_RELEASE.json"):
        ...
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F3Release.v0.1",
            "release":R6F,
            "source_commit":source_commit,
    ...
    wt(root/"P10_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX_VALIDATION.txt",f"""ConceptGhost P10 Gate 7 R6F3 Route Editor Final
...
def main(root):
 ...
 print('CONCEPTGHOST_GATE7_R6F3_FLEXGEMM_TRITON32_COMPAT_FIX_PASS'); return 0
...
def rebuild(root,source_commit):
    ...
    for name in ("BUNDLE_MANIFEST.json","P10_DR9_BUNDLE_MANIFEST.json"):
        ...
        data=json.loads(rt(path))
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F3Bundle.v0.1",
            "release":R6F,
            "source_commit":source_commit,

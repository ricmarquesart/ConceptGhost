...
R6F="ConceptGhost_v1.54_P10_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX"
...
def patch_entrypoints(root):
    ...
    replacements=(
        ("Gate 7 Preview r5 + Geometric Evidence + Run-Local Audit","Gate 7 R6F5 + FlexGEMM callsite/Triton 3.2 Compatibility Fix"),
        (R5,R6F),
        ("before_gate7_preview_r5_geometric_evidence_audit_fix","before_gate7_r6f5_flexgemm_callsite_compat_fix"),
        ("P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_READY_VERIFY.json","P10_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX_READY_VERIFY.json"),
        ("CONCEPTGHOST_P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS","CONCEPTGHOST_P10_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX_RUNTIME_VERIFY_PASS"),
    )
    for old,new in replacements:
        install=install.replace(old,new)
        verify=verify.replace(old,new)
    wt(root/"Installer/install_gate7_r6f5_flexgemm_callsite_compat_fix.ps1",install)
    wt(root/"Installer/verify_gate7_r6f5_flexgemm_callsite_compat_fix.ps1",verify)
    old_install.unlink()
    old_verify.unlink()
...
cd /d "%~dp0"
echo ============================================================
echo 03 - ConceptGhost P10 Gate 7 R6F5
...
echo ============================================================
if not exist "%~dp0Installer\\install_gate7_r6f5_flexgemm_callsite_compat_fix.ps1" (
  echo [FAIL] Required R6F installer entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\install_gate7_r6f5_flexgemm_callsite_compat_fix.ps1"
if errorlevel 1 (
...
cd /d "%~dp0"
echo ============================================================
echo 04 - VERIFY ConceptGhost P10 Gate 7 R6F5
echo ============================================================
if not exist "%~dp0Installer\\verify_gate7_r6f5_flexgemm_callsite_compat_fix.ps1" (
  echo [FAIL] Required R6F verifier entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\verify_gate7_r6f5_flexgemm_callsite_compat_fix.ps1"
if errorlevel 1 (
...
def patch_production_workflow(root):
    ...
    workflow=json.loads(rt(path))
    extra=workflow.setdefault("extra",{}).setdefault("conceptghost",{})
    extra["gate7_preview_release"]="GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX"
...
def main() -> int:
    ...
    report = {
        "schema": "ConceptGhost.FlexGEMMTriton32Compatibility.v0.3",
        "status": "FAIL",
        "runtime_root": str(runtime_root),
        "scope": "CONCEPTGHOST_PRIVATE_MOGE_RUNTIME_ONLY",
        "upstream_flexgemm_commit": UPSTREAM_COMMIT,
        "policy": "PATCH_ONLY_ACTIVE_INT32_CALLSITES",
    ...
    # FlexGEMM host wrappers already convert keys/queries to torch.int32 and pass
    # D as the number of int32 words. Patch only live Triton callsites; leave the
    # generic helper definition untouched because the patched lookup no longer calls it.
    build_variants = (
    ...
    build_safe = (
        '    # ConceptGhost R6F5: host wrapper already passes torch.int32-packed keys.\\n'
        '    keys_ptr_32 = tl.cast(keys_ptr, tl.pointer_type(tl.int32))\\n'
        '    D_32: tl.constexpr = D\\n'
    )

    safe_count = patched.count(build_safe)
    variant_count = sum(patched.count(v) for v in build_variants)
    if safe_count == 2:
        transitions.append({"label": "build_unique", "state": "ALREADY_PATCHED", "count": 2})
    elif safe_count == 0 and variant_count == 2:
        for variant in build_variants:
            patched = patched.replace(variant, build_safe)
        transitions.append({"label": "build_unique", "state": "PATCHED", "count": 2})
    else:
        report["error"] = f"Unexpected build/unique callsite shape: safe={safe_count} variants={variant_count}"
    ...
    safe_lookup_count = patched.count(lookup_safe)
    variant_lookup_count = sum(patched.count(v) for v in lookup_variants)
    if safe_lookup_count == 1:
        transitions.append({"label": "lookup_inline", "state": "ALREADY_PATCHED", "count": 1})
    elif safe_lookup_count == 0 and variant_lookup_count == 1:
        for variant in lookup_variants:
            patched = patched.replace(variant, lookup_safe)
        transitions.append({"label": "lookup_inline", "state": "PATCHED", "count": 1})
    else:
        report["error"] = f"Unexpected lookup callsite shape: safe={safe_lookup_count} variants={variant_lookup_count}"
        return emit(report_path, report, 5)

    # The helper may still contain dtype.itemsize/primitive_bitwidth, but it is
    # now dead for hashmap lookup. Prove that every active callsite uses D directly.
    if patched.count(build_safe) != 2 or patched.count(lookup_safe) != 1:
        report["error"] = "Active callsite replacement count mismatch"
    ...
    if patched != original:
        backup = target.with_suffix(target.suffix + ".conceptghost_pre_r6f5")
    ...
    if cc < (8,0):
        if _minor(torch.__version__) != (2,6) or _minor(triton_version) != (3,2):
            raise RuntimeError(
                ...
                "the private runtime torch 2.6.x + Triton 3.2.x. Re-run the R6F5 installer. "
...
'''
    ...
    lock["release"]="ConceptGhost v1.54 R6F5 MoGe3 Turing Runtime Fix"
    ...
    guide=root/"USER_GUIDE_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX.md"
    wt(guide,rt(guide)+"""
## R6F5 MoGe-3 Turing runtime correction
...
R6F5 applies a bounded compatibility bridge to the ConceptGhost-private FlexGEMM hashmap kernels. FlexGEMM's Python wrappers already serialize keys and queries into padded torch.int32 tensors and pass D as the number of int32 words, so R6F5 removes Triton-JIT dtype-width introspection from those kernels and consumes that existing host-side packing contract directly. Model weights, resolution, refine steps and geometry authority are unchanged.
""")

    validation=root/"P10_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX_VALIDATION.txt"
    wt(validation,rt(validation)+"""
R6F5 MoGe-3 Turing runtime compatibility:
...
""")
    ...
    test=root/"Installer/test_r6f5_flexgemm_callsite_compat_fix.py"
...
def main(root):
    ...
    for token in ("PATCH_ONLY_ACTIVE_INT32_CALLSITES","build_safe","lookup_safe","D_32: tl.constexpr = D","query_vec_32 = query_vec.to(tl.int32)","FLEXGEMM_TRITON32_PATCH.json"):
        if token not in patch: errors.append("FlexGEMM callsite bridge missing "+token)
    try:
    ...
    print("CONCEPTGHOST_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX_PASS"); return 0
...
def patch_contracts(root,source_commit,package_commit):
    ...
    wt(p,rt(p).replace(
        ...
        "CONCEPTGHOST_P10_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX_RUNTIME_VERIFY_PASS",
    ))

    p=root/"Installer/test_gate7_preview_bundle.py"
    text=rt(p).replace(
        "GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS",
        "GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX_RUNTIME_VERIFY_PASS",
    ).replace(
        "Installer/install_gate7_preview_r5_geometric_evidence_audit_fix.ps1",
        "Installer/install_gate7_r6f5_flexgemm_callsite_compat_fix.ps1",
    ).replace(
        "Installer/verify_gate7_preview_r5_geometric_evidence_audit_fix.ps1",
        "Installer/verify_gate7_r6f5_flexgemm_callsite_compat_fix.ps1",
    )
    wt(p,text)
    ...
    wt(p,rt(p).replace(
        ...
        "USER_GUIDE_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX.md",
    ))
    ...
    guide=rt(old_guide).replace(
        ...
        "Gate 7 R6F5 + FlexGEMM callsite/Triton 3.2 Compatibility Fix",
    )
    guide += """
...
"""
    wt(root/"USER_GUIDE_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX.md",guide)
    old_guide.unlink()

    wt(root/"Payload/docs/40_R6F_ROUTE_EDITOR_FINAL.md",f"""# ConceptGhost P10 Gate 7 R6F5 — Route Editor Final
...
""")
    ...
    data.update({
        "schema":"ConceptGhost.P10Gate7R6F5CodeManifest.v0.1",
        "release":R6F,
    ...
    for name in ("RELEASE.json","P10_DR9_RELEASE.json"):
        ...
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F5Release.v0.1",
            "release":R6F,
            "source_commit":source_commit,
    ...
    wt(root/"P10_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX_VALIDATION.txt",f"""ConceptGhost P10 Gate 7 R6F5 Route Editor Final
...
def main(root):
 ...
 print('CONCEPTGHOST_GATE7_R6F5_FLEXGEMM_CALLSITE_COMPAT_FIX_PASS'); return 0
...
def rebuild(root,source_commit):
    ...
    for name in ("BUNDLE_MANIFEST.json","P10_DR9_BUNDLE_MANIFEST.json"):
        ...
        data=json.loads(rt(path))
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F5Bundle.v0.1",
            "release":R6F,
            "source_commit":source_commit,
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="cg-r6f-") as temp:
        ...
        tests=(
            ...
            "test_r6f5_flexgemm_callsite_compat_fix.py",
        )
        for name in tests:

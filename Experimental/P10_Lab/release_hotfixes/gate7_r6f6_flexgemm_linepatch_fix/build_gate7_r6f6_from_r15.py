...
R6F="ConceptGhost_v1.54_P10_GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX"
...
def patch_entrypoints(root):
    ...
    replacements=(
        ("Gate 7 Preview r5 + Geometric Evidence + Run-Local Audit","Gate 7 R6F6 + FlexGEMM line-patch/Triton 3.2 Compatibility Fix"),
        (R5,R6F),
        ("before_gate7_preview_r5_geometric_evidence_audit_fix","before_gate7_r6f6_flexgemm_linepatch_fix"),
        ("P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_READY_VERIFY.json","P10_GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX_READY_VERIFY.json"),
        ("CONCEPTGHOST_P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS","CONCEPTGHOST_P10_GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX_RUNTIME_VERIFY_PASS"),
    )
    for old,new in replacements:
        install=install.replace(old,new)
        verify=verify.replace(old,new)
    wt(root/"Installer/install_gate7_r6f6_flexgemm_linepatch_fix.ps1",install)
    wt(root/"Installer/verify_gate7_r6f6_flexgemm_linepatch_fix.ps1",verify)
    old_install.unlink()
    old_verify.unlink()
...
cd /d "%~dp0"
echo ============================================================
echo 03 - ConceptGhost P10 Gate 7 R6F6
...
echo ============================================================
if not exist "%~dp0Installer\\install_gate7_r6f6_flexgemm_linepatch_fix.ps1" (
  echo [FAIL] Required R6F installer entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\install_gate7_r6f6_flexgemm_linepatch_fix.ps1"
if errorlevel 1 (
...
cd /d "%~dp0"
echo ============================================================
echo 04 - VERIFY ConceptGhost P10 Gate 7 R6F6
echo ============================================================
if not exist "%~dp0Installer\\verify_gate7_r6f6_flexgemm_linepatch_fix.ps1" (
  echo [FAIL] Required R6F verifier entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\verify_gate7_r6f6_flexgemm_linepatch_fix.ps1"
if errorlevel 1 (
...
def patch_production_workflow(root):
    ...
    workflow=json.loads(rt(path))
    extra=workflow.setdefault("extra",{}).setdefault("conceptghost",{})
    extra["gate7_preview_release"]="GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX"
...
def _replace_line_family(text: str, patterns: tuple[str, ...], replacement: str, expected_total: int, label: str):
    total = 0
...
def main() -> int:
    ...
    report = {
        "schema": "ConceptGhost.FlexGEMMTriton32Compatibility.v0.4",
        "status": "FAIL",
        "runtime_root": str(runtime_root),
        "scope": "CONCEPTGHOST_PRIVATE_MOGE_RUNTIME_ONLY",
        "upstream_flexgemm_commit": UPSTREAM_COMMIT,
        "policy": "PATCH_ACTIVE_INT32_CALLSITES_BY_LINE",
    ...
    # R6F5 failed because its generated matcher compared literal "\\n" sequences
    # against real source newlines. R6F6 intentionally patches individual source
    # lines with regex and verifies exact occurrence counts.
    assert_patterns = (
        r'^[ \t]*tl\.static_assert\(D \* keys_ptr\.dtype\.element_ty\.itemsize % 4 == 0, "keys byte width must be divisible by 4"\)[ \t]*$',
        r'^[ \t]*tl\.static_assert\(D \* \(keys_ptr\.dtype\.element_ty\.primitive_bitwidth // 8\) % 4 == 0, "keys byte width must be divisible by 4"\)[ \t]*$',
    )
    safe_assert_comment = '    # ConceptGhost R6F6: host already passes torch.int32-packed keys; byte-width assert is redundant.'
    patched, assert_count = _replace_line_family(patched, assert_patterns, safe_assert_comment, 2, "build_assert")
    existing_assert_safe = patched.count(safe_assert_comment)
    if assert_count == 0 and existing_assert_safe == 2:
        transitions.append({"label": "build_assert", "state": "ALREADY_PATCHED", "count": 2})
    elif assert_count == 2:
        transitions.append({"label": "build_assert", "state": "PATCHED", "count": 2})
    else:
        report["error"] = f"Unexpected build/unique assert count: patched={assert_count} safe={existing_assert_safe}"
        return emit(report_path, report, 4)

    d_patterns = (
        r'^[ \t]*D_32: tl\.constexpr = D \* keys_ptr\.dtype\.element_ty\.itemsize // 4[ \t]*$',
        r'^[ \t]*D_32: tl\.constexpr = D \* \(keys_ptr\.dtype\.element_ty\.primitive_bitwidth // 8\) // 4[ \t]*$',
    )
    safe_d = '    D_32: tl.constexpr = D'
    before_safe_d = patched.count(safe_d)
    patched, d_count = _replace_line_family(patched, d_patterns, safe_d, 2, "build_d")
    after_safe_d = patched.count(safe_d)
    # Three safe D lines are expected after full repair: two build/unique + one lookup.
    if d_count not in (0, 2):
        report["error"] = f"Unexpected build/unique D_32 replacement count: {d_count}"
        return emit(report_path, report, 5)
    transitions.append({"label": "build_d", "state": "PATCHED" if d_count else "ALREADY_PATCHED", "count": d_count})

    lookup_pack_patterns = (
        r'^[ \t]*query_vec_32 = _vec_pack_little_endian_to_int32\(query_vec\)[ \t]*$',
    )
    safe_lookup_pack = '    query_vec_32 = query_vec.to(tl.int32)'
    patched, lookup_pack_count = _replace_line_family(
        patched, lookup_pack_patterns, safe_lookup_pack, 1, "lookup_pack"
    )
    if lookup_pack_count == 0 and patched.count(safe_lookup_pack) == 1:
        transitions.append({"label": "lookup_pack", "state": "ALREADY_PATCHED", "count": 1})
    elif lookup_pack_count == 1:
        transitions.append({"label": "lookup_pack", "state": "PATCHED", "count": 1})
    else:
        report["error"] = f"Unexpected lookup pack count: {lookup_pack_count}"
        return emit(report_path, report, 6)

    lookup_d_patterns = (
        r'^[ \t]*D_32: tl\.constexpr = D \* query_vec\.dtype\.itemsize // 4[ \t]*$',
        r'^[ \t]*D_32: tl\.constexpr = D \* \(query_vec\.dtype\.primitive_bitwidth // 8\) // 4[ \t]*$',
    )
    # Replace only a remaining lookup D line. If already safe, the total safe D
    # count will already include it.
    patched, lookup_d_count = _replace_line_family(patched, lookup_d_patterns, safe_d, 1, "lookup_d")
    if lookup_d_count not in (0, 1):
        report["error"] = f"Unexpected lookup D_32 replacement count: {lookup_d_count}"
        return emit(report_path, report, 7)
    transitions.append({"label": "lookup_d", "state": "PATCHED" if lookup_d_count else "ALREADY_PATCHED", "count": lookup_d_count})

    if patched.count(safe_d) != 3:
        report["error"] = f"Expected exactly three active D_32=D lines; found {patched.count(safe_d)}"
        return emit(report_path, report, 8)
    if patched.count(safe_lookup_pack) != 1:
        report["error"] = f"Expected exactly one active int32 lookup cast; found {patched.count(safe_lookup_pack)}"
        return emit(report_path, report, 9)
    if "_vec_pack_little_endian_to_int32(query_vec)" in patched:
        report["error"] = "Active lookup still calls dtype-introspecting helper"
        return emit(report_path, report, 10)
    ...
    if missing_host:
        ...
        return emit(report_path, report, 11)

    if patched != original:
        backup = target.with_suffix(target.suffix + ".conceptghost_pre_r6f6")
    ...
    report["host_contract"] = {
        ...
        "active_kernel_dtype_introspection_required": False,
    }
    report["status"] = "PASS"
    ...
    if cc < (8,0):
        if _minor(torch.__version__) != (2,6) or _minor(triton_version) != (3,2):
            raise RuntimeError(
                ...
                "the private runtime torch 2.6.x + Triton 3.2.x. Re-run the R6F6 installer. "
...
'''
    ...
    lock["release"]="ConceptGhost v1.54 R6F6 MoGe3 Turing Runtime Fix"
    ...
    guide=root/"USER_GUIDE_GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX.md"
    wt(guide,rt(guide)+"""
## R6F6 MoGe-3 Turing runtime correction
...
R6F6 applies a bounded compatibility bridge to the ConceptGhost-private FlexGEMM hashmap kernels. FlexGEMM's Python wrappers already serialize keys and queries into padded torch.int32 tensors and pass D as the number of int32 words, so R6F6 removes Triton-JIT dtype-width introspection from those kernels and consumes that existing host-side packing contract directly. Model weights, resolution, refine steps and geometry authority are unchanged.
""")

    validation=root/"P10_GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX_VALIDATION.txt"
    wt(validation,rt(validation)+"""
R6F6 MoGe-3 Turing runtime compatibility:
...
""")
    ...
    test=root/"Installer/test_r6f6_flexgemm_linepatch_fix.py"
...
def main(root):
    ...
    print("CONCEPTGHOST_R6F6_FLEXGEMM_LINEPATCH_FIX_PASS"); return 0
...
def patch_contracts(root,source_commit,package_commit):
    ...
    wt(p,rt(p).replace(
        ...
        "USER_GUIDE_GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX.md",
    ))
    ...
    guide=rt(old_guide).replace(
        ...
        "Gate 7 R6F6 + FlexGEMM line-patch/Triton 3.2 Compatibility Fix",
    )
    guide += """
...
"""
    wt(root/"USER_GUIDE_GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX.md",guide)
    old_guide.unlink()

    wt(root/"Payload/docs/40_R6F_ROUTE_EDITOR_FINAL.md",f"""# ConceptGhost P10 Gate 7 R6F6 — Route Editor Final
...
""")
    ...
    data.update({
        "schema":"ConceptGhost.P10Gate7R6F6CodeManifest.v0.1",
        "release":R6F,
    ...
    for name in ("RELEASE.json","P10_DR9_RELEASE.json"):
        ...
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F6Release.v0.1",
            "release":R6F,
            "source_commit":source_commit,
    ...
    wt(root/"P10_GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX_VALIDATION.txt",f"""ConceptGhost P10 Gate 7 R6F6 Route Editor Final
...
def main(root):
 ...
 print('CONCEPTGHOST_GATE7_R6F6_FLEXGEMM_LINEPATCH_FIX_PASS'); return 0
...
def rebuild(root,source_commit):
    ...
    for name in ("BUNDLE_MANIFEST.json","P10_DR9_BUNDLE_MANIFEST.json"):
        ...
        data=json.loads(rt(path))
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F6Bundle.v0.1",
            "release":R6F,
            "source_commit":source_commit,
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="cg-r6f-") as temp:
        ...
        tests=(
            ...
            "test_r6f6_flexgemm_linepatch_fix.py",
        )
        for name in tests:

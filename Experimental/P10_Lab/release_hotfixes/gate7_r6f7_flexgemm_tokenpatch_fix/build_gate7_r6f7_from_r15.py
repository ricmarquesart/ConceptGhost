...
R6F="ConceptGhost_v1.54_P10_GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX"
...
def patch_entrypoints(root):
    ...
    replacements=(
        ("Gate 7 Preview r5 + Geometric Evidence + Run-Local Audit","Gate 7 R6F7 + FlexGEMM token-patch/Triton 3.2 Compatibility Fix"),
        (R5,R6F),
        ("before_gate7_preview_r5_geometric_evidence_audit_fix","before_gate7_r6f7_flexgemm_tokenpatch_fix"),
        ("P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_READY_VERIFY.json","P10_GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX_READY_VERIFY.json"),
        ("CONCEPTGHOST_P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS","CONCEPTGHOST_P10_GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX_RUNTIME_VERIFY_PASS"),
    )
    for old,new in replacements:
        install=install.replace(old,new)
        verify=verify.replace(old,new)
    wt(root/"Installer/install_gate7_r6f7_flexgemm_tokenpatch_fix.ps1",install)
    wt(root/"Installer/verify_gate7_r6f7_flexgemm_tokenpatch_fix.ps1",verify)
    old_install.unlink()
    old_verify.unlink()
...
cd /d "%~dp0"
echo ============================================================
echo 03 - ConceptGhost P10 Gate 7 R6F7
...
echo ============================================================
if not exist "%~dp0Installer\\install_gate7_r6f7_flexgemm_tokenpatch_fix.ps1" (
  echo [FAIL] Required R6F installer entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\install_gate7_r6f7_flexgemm_tokenpatch_fix.ps1"
if errorlevel 1 (
...
cd /d "%~dp0"
echo ============================================================
echo 04 - VERIFY ConceptGhost P10 Gate 7 R6F7
echo ============================================================
if not exist "%~dp0Installer\\verify_gate7_r6f7_flexgemm_tokenpatch_fix.ps1" (
  echo [FAIL] Required R6F verifier entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\verify_gate7_r6f7_flexgemm_tokenpatch_fix.ps1"
if errorlevel 1 (
...
def patch_production_workflow(root):
    ...
    workflow=json.loads(rt(path))
    extra=workflow.setdefault("extra",{}).setdefault("conceptghost",{})
    extra["gate7_preview_release"]="GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX"
...
def patch_text(source: str) -> tuple[str, dict]:
    """Patch semantic tokens, never line endings or multiline formatting.

    The FlexGEMM host wrappers already pass torch.int32-packed keys/queries and
    D as the number of int32 words. We therefore remove only the active JIT
    dtype-width dependency. This is invariant to LF/CRLF and whitespace.
    """
    patched = source
    stats = {}
    ...
    # Normalize the three active D expressions after width has become literal 4.
    d_expr = "D * 4 // 4"
...
def self_test() -> int:
    ...
    for label, fixture in (
        ("pristine_lf", pristine),
        ("pristine_crlf", pristine.replace("\\n", "\\r\\n")),
        ("prior_lf", prior),
        ("prior_crlf", prior.replace("\\n", "\\r\\n")),
    ):
        patched, stats = patch_text(fixture)
        if patched.count("D_32: tl.constexpr = D") != 3:
            raise RuntimeError(label + ": D normalization failed")
        if patched.count("query_vec.to(tl.int32)") != 1:
            raise RuntimeError(label + ": lookup normalization failed")
        patched2, _stats2 = patch_text(patched)
        if patched2 != patched:
            raise RuntimeError(label + ": patch is not idempotent")
    print("CONCEPTGHOST_FLEXGEMM_TOKENPATCH_SELFTEST_PASS")
    return 0
...
def main() -> int:
    ...
    report = {
        "schema": "ConceptGhost.FlexGEMMTriton32Compatibility.v0.5",
        "status": "FAIL",
        "runtime_root": str(runtime_root),
        "scope": "CONCEPTGHOST_PRIVATE_MOGE_RUNTIME_ONLY",
        "upstream_flexgemm_commit": UPSTREAM_COMMIT,
        "policy": "PATCH_ACTIVE_INT32_TOKENS_NOT_LINES",
    ...
    try:
        ...
        report["source_probe"] = {
            "key_itemsize_count": original.count("keys_ptr.dtype.element_ty.itemsize"),
            "key_primitive_count": original.count("keys_ptr.dtype.element_ty.primitive_bitwidth"),
            "query_itemsize_count": original.count("query_vec.dtype.itemsize"),
            "query_primitive_count": original.count("query_vec.dtype.primitive_bitwidth"),
            "legacy_lookup_call_count": original.count("_vec_pack_little_endian_to_int32(query_vec)"),
            "safe_lookup_count": original.count("query_vec.to(tl.int32)"),
            "safe_d_count": original.count("D_32: tl.constexpr = D"),
        }
        return emit(report_path, report, 4)

    if patched != original:
        backup = target.with_suffix(target.suffix + ".conceptghost_pre_r6f7")
    ...
    if cc < (8,0):
        if _minor(torch.__version__) != (2,6) or _minor(triton_version) != (3,2):
            raise RuntimeError(
                ...
                "the private runtime torch 2.6.x + Triton 3.2.x. Re-run the R6F7 installer. "
...
'''
    ...
    lock["release"]="ConceptGhost v1.54 R6F7 MoGe3 Turing Runtime Fix"
    ...
    guide=root/"USER_GUIDE_GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX.md"
    wt(guide,rt(guide)+"""
## R6F7 MoGe-3 Turing runtime correction
...
R6F7 applies a bounded compatibility bridge to the ConceptGhost-private FlexGEMM hashmap kernels. FlexGEMM's Python wrappers already serialize keys and queries into padded torch.int32 tensors and pass D as the number of int32 words, so R6F7 removes Triton-JIT dtype-width introspection from those kernels and consumes that existing host-side packing contract directly. Model weights, resolution, refine steps and geometry authority are unchanged.
""")

    validation=root/"P10_GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX_VALIDATION.txt"
    wt(validation,rt(validation)+"""
R6F7 MoGe-3 Turing runtime compatibility:
...
""")
    ...
    test=root/"Installer/test_r6f7_flexgemm_tokenpatch_fix.py"
...
def main(root):
    ...
    for token in ("PATCH_ACTIVE_INT32_TOKENS_NOT_LINES","def patch_text","--self-test","source_probe","D_32: tl.constexpr = D","FLEXGEMM_TRITON32_PATCH.json"):
        if token not in patch: errors.append("FlexGEMM token bridge missing "+token)
    try:
    ...
    if probe.returncode != 0 or "CONCEPTGHOST_FLEXGEMM_TOKENPATCH_SELFTEST_PASS" not in probe.stdout:
        errors.append("FlexGEMM token bridge self-test failed: "+probe.stdout+" "+probe.stderr)
    ...
    print("CONCEPTGHOST_R6F7_FLEXGEMM_TOKENPATCH_FIX_PASS"); return 0
...
def patch_contracts(root,source_commit,package_commit):
    ...
    wt(p,rt(p).replace(
        ...
        "USER_GUIDE_GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX.md",
    ))
    ...
    guide=rt(old_guide).replace(
        ...
        "Gate 7 R6F7 + FlexGEMM token-patch/Triton 3.2 Compatibility Fix",
    )
    guide += """
...
"""
    wt(root/"USER_GUIDE_GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX.md",guide)
    old_guide.unlink()

    wt(root/"Payload/docs/40_R6F_ROUTE_EDITOR_FINAL.md",f"""# ConceptGhost P10 Gate 7 R6F7 — Route Editor Final
...
""")
    ...
    data.update({
        "schema":"ConceptGhost.P10Gate7R6F7CodeManifest.v0.1",
        "release":R6F,
    ...
    for name in ("RELEASE.json","P10_DR9_RELEASE.json"):
        ...
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F7Release.v0.1",
            "release":R6F,
            "source_commit":source_commit,
    ...
    wt(root/"P10_GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX_VALIDATION.txt",f"""ConceptGhost P10 Gate 7 R6F7 Route Editor Final
...
def main(root):
 ...
 print('CONCEPTGHOST_GATE7_R6F7_FLEXGEMM_TOKENPATCH_FIX_PASS'); return 0
...
def rebuild(root,source_commit):
    ...
    for name in ("BUNDLE_MANIFEST.json","P10_DR9_BUNDLE_MANIFEST.json"):
        ...
        data=json.loads(rt(path))
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F7Bundle.v0.1",
            "release":R6F,
            "source_commit":source_commit,
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="cg-r6f-") as temp:
        ...
        tests=(
            ...
            "test_r6f7_flexgemm_tokenpatch_fix.py",
        )
        for name in tests:

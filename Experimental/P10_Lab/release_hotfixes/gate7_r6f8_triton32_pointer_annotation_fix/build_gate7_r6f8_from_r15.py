...
R6F="ConceptGhost_v1.54_P10_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX"
...
def patch_entrypoints(root):
    ...
    replacements=(
        ("Gate 7 Preview r5 + Geometric Evidence + Run-Local Audit","Gate 7 R6F8 + Triton 3.2 Pointer Annotation Compatibility Fix"),
        (R5,R6F),
        ("before_gate7_preview_r5_geometric_evidence_audit_fix","before_gate7_r6f8_triton32_pointer_annotation_fix"),
        ("P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_READY_VERIFY.json","P10_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX_READY_VERIFY.json"),
        ("CONCEPTGHOST_P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS","CONCEPTGHOST_P10_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX_RUNTIME_VERIFY_PASS"),
    )
    for old,new in replacements:
        install=install.replace(old,new)
        verify=verify.replace(old,new)
    wt(root/"Installer/install_gate7_r6f8_triton32_pointer_annotation_fix.ps1",install)
    wt(root/"Installer/verify_gate7_r6f8_triton32_pointer_annotation_fix.ps1",verify)
    old_install.unlink()
    old_verify.unlink()
...
cd /d "%~dp0"
echo ============================================================
echo 03 - ConceptGhost P10 Gate 7 R6F8
...
echo ============================================================
if not exist "%~dp0Installer\\install_gate7_r6f8_triton32_pointer_annotation_fix.ps1" (
  echo [FAIL] Required R6F installer entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\install_gate7_r6f8_triton32_pointer_annotation_fix.ps1"
if errorlevel 1 (
...
cd /d "%~dp0"
echo ============================================================
echo 04 - VERIFY ConceptGhost P10 Gate 7 R6F8
echo ============================================================
if not exist "%~dp0Installer\\verify_gate7_r6f8_triton32_pointer_annotation_fix.ps1" (
  echo [FAIL] Required R6F verifier entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\verify_gate7_r6f8_triton32_pointer_annotation_fix.ps1"
if errorlevel 1 (
...
def patch_production_workflow(root):
    ...
    workflow=json.loads(rt(path))
    extra=workflow.setdefault("extra",{}).setdefault("conceptghost",{})
    extra["gate7_preview_release"]="GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX"
...
def strip_pointer_annotations(source: str) -> tuple[str, int]:
    """Remove only the Triton annotation known to break Triton 3.2 signatures.

    Triton 3.2 can infer pointer types from runtime tensor arguments. Explicit
    ': tl.pointer_type' annotations are parsed as a signature token ending in
    'ier_type' and fail in str_to_ty. tl.constexpr annotations are preserved.
    """
    count_union = source.count(": tl.pointer_type | None")
    source = source.replace(": tl.pointer_type | None", "")
    count_plain = source.count(": tl.pointer_type")
    source = source.replace(": tl.pointer_type", "")
    return source, count_union + count_plain


def self_test() -> int:
    fixture = (
        "@triton.jit\\n"
        "def kernel(ptr: tl.pointer_type, maybe: tl.pointer_type | None, BLOCK: tl.constexpr):\\n"
        "    return\\n"
    )
    patched, count = strip_pointer_annotations(fixture)
    if count != 2:
        raise RuntimeError("pointer annotation self-test count mismatch")
    if "tl.pointer_type" in patched:
        raise RuntimeError("pointer annotation self-test left unsupported annotation")
    if "BLOCK: tl.constexpr" not in patched:
        raise RuntimeError("pointer annotation self-test damaged constexpr")
    patched2, count2 = strip_pointer_annotations(patched)
    if patched2 != patched or count2 != 0:
        raise RuntimeError("pointer annotation patch is not idempotent")
    ...
    print("CONCEPTGHOST_FLEXGEMM_TRITON32_R6F8_SELFTEST_PASS")
    return 0
...
def main() -> int:
    ...
    report = {
        "schema": "ConceptGhost.FlexGEMMTriton32Compatibility.v0.6",
        "status": "FAIL",
        "runtime_root": str(runtime_root),
        "scope": "CONCEPTGHOST_PRIVATE_MOGE_RUNTIME_ONLY",
        "upstream_flexgemm_commit": UPSTREAM_COMMIT,
        "policy": "R6F7_HASHMAP_TOKENPATCH_PLUS_REMOVE_TL_POINTER_TYPE_ANNOTATIONS",
    ...
    backups = []
    try:
        # Preserve the successful R6F7 hashmap compatibility bridge.
        ...
        if patched_hashmap != original_hashmap:
            backup = hashmap.with_suffix(hashmap.suffix + ".conceptghost_pre_r6f8")
        ...
        # Remove explicit tl.pointer_type annotations from every private
        # FlexGEMM Triton kernel module. This addresses the exact Triton 3.2
        # signature parser failure KeyError('ier_type').
        pointer_stats = {}
        for path in sorted(triton_root.rglob("*.py")):
            source = path.read_text(encoding="utf-8")
            patched, count = strip_pointer_annotations(source)
            pointer_stats[str(path.relative_to(package_root))] = count
            if patched != source:
                backup = path.with_suffix(path.suffix + ".conceptghost_pre_r6f8")
                if not backup.exists():
        ...
        for path in sorted(triton_root.rglob("*.py")):
            ...
            if ": tl.pointer_type" in text:
                remaining.append(str(path))
        if remaining:
            raise RuntimeError("unsupported tl.pointer_type annotations remain: " + ", ".join(remaining))

        for path in sorted(set(Path(item) for item in touched)):
            py_compile.compile(str(path), doraise=True)

        report["hashmap_stats"] = hashmap_stats
        report["pointer_annotation_replacements"] = pointer_stats
        report["pointer_annotation_total"] = sum(pointer_stats.values())
        report["touched_files"] = sorted(set(touched))
        report["backups"] = sorted(set(backups))
        report["triton32_signature_fix"] = {
            "removed_annotation": "tl.pointer_type",
            "preserved_annotation": "tl.constexpr",
            "expected_failure_removed": "KeyError('ier_type')",
        }
        report["status"] = "PASS"
    ...
    if cc < (8,0):
        if _minor(torch.__version__) != (2,6) or _minor(triton_version) != (3,2):
            raise RuntimeError(
                ...
                "the private runtime torch 2.6.x + Triton 3.2.x. Re-run the R6F8 installer. "
...
'''
    ...
    lock["release"]="ConceptGhost v1.54 R6F8 MoGe3 Turing Runtime Fix"
    ...
    guide=root/"USER_GUIDE_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX.md"
    wt(guide,rt(guide)+"""
## R6F8 MoGe-3 Turing runtime correction
...
R6F8 applies a bounded compatibility bridge to the ConceptGhost-private FlexGEMM hashmap kernels. FlexGEMM's Python wrappers already serialize keys and queries into padded torch.int32 tensors and pass D as the number of int32 words, so R6F8 removes Triton-JIT dtype-width introspection from those kernels and consumes that existing host-side packing contract directly. Model weights, resolution, refine steps and geometry authority are unchanged.
""")

    validation=root/"P10_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX_VALIDATION.txt"
    wt(validation,rt(validation)+"""
R6F8 MoGe-3 Turing runtime compatibility:
...
""")
    ...
    test=root/"Installer/test_r6f8_triton32_pointer_annotation_fix.py"
...
def main(root):
    ...
    for token in ("REMOVE_TL_POINTER_TYPE_ANNOTATIONS","strip_pointer_annotations","KeyError('ier_type')","preserved_annotation","--self-test"):
        if token not in patch: errors.append("R6F8 pointer-annotation bridge missing "+token)
    try:
        compile(patch, str(patch_path), "exec")
    except SyntaxError as exc:
        errors.append("R6F8 bridge syntax error "+repr(exc))
    probe=subprocess.run([sys.executable,str(patch_path),"--self-test"],capture_output=True,text=True)
    if probe.returncode != 0 or "CONCEPTGHOST_FLEXGEMM_TRITON32_R6F8_SELFTEST_PASS" not in probe.stdout:
        errors.append("R6F8 bridge self-test failed: "+probe.stdout+" "+probe.stderr)
    ...
    print("CONCEPTGHOST_R6F8_TRITON32_POINTER_ANNOTATION_FIX_PASS"); return 0
...
def patch_contracts(root,source_commit,package_commit):
    ...
    wt(p,rt(p).replace(
        ...
        "CONCEPTGHOST_P10_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX_RUNTIME_VERIFY_PASS",
    ))

    p=root/"Installer/test_gate7_preview_bundle.py"
    text=rt(p).replace(
        "GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS",
        "GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX_RUNTIME_VERIFY_PASS",
    ).replace(
        "Installer/install_gate7_preview_r5_geometric_evidence_audit_fix.ps1",
        "Installer/install_gate7_r6f8_triton32_pointer_annotation_fix.ps1",
    ).replace(
        "Installer/verify_gate7_preview_r5_geometric_evidence_audit_fix.ps1",
        "Installer/verify_gate7_r6f8_triton32_pointer_annotation_fix.ps1",
    )
    wt(p,text)
    ...
    wt(p,rt(p).replace(
        ...
        "USER_GUIDE_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX.md",
    ))
...
"""
    wt(root/"USER_GUIDE_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX.md",guide)
    old_guide.unlink()

    wt(root/"Payload/docs/40_R6F_ROUTE_EDITOR_FINAL.md",f"""# ConceptGhost P10 Gate 7 R6F8 — Route Editor Final
...
""")
    ...
    data.update({
        "schema":"ConceptGhost.P10Gate7R6F8CodeManifest.v0.1",
        "release":R6F,
    ...
    for name in ("RELEASE.json","P10_DR9_RELEASE.json"):
        ...
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F8Release.v0.1",
            "release":R6F,
            "source_commit":source_commit,
    ...
    wt(root/"P10_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX_VALIDATION.txt",f"""ConceptGhost P10 Gate 7 R6F8 Route Editor Final
...
def main(root):
 ...
 print('CONCEPTGHOST_GATE7_R6F8_TRITON32_POINTER_ANNOTATION_FIX_PASS'); return 0
...
def rebuild(root,source_commit):
    ...
    for name in ("BUNDLE_MANIFEST.json","P10_DR9_BUNDLE_MANIFEST.json"):
        ...
        data=json.loads(rt(path))
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F8Bundle.v0.1",
            "release":R6F,
            "source_commit":source_commit,
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="cg-r6f-") as temp:
        ...
        tests=(
            ...
            "test_r6f8_triton32_pointer_annotation_fix.py",
        )
        for name in tests:

...
R6F="ConceptGhost_v1.54_P10_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX"
...
def patch_entrypoints(root):
    ...
    replacements=(
        ("Gate 7 Preview r5 + Geometric Evidence + Run-Local Audit","Gate 7 R6F9 + Triton 3.2 Annotation Sanitizer Fix"),
        (R5,R6F),
        ("before_gate7_preview_r5_geometric_evidence_audit_fix","before_gate7_r6f9_triton32_annotation_sanitizer_fix"),
        ("P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_READY_VERIFY.json","P10_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX_READY_VERIFY.json"),
        ("CONCEPTGHOST_P10_GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS","CONCEPTGHOST_P10_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX_RUNTIME_VERIFY_PASS"),
    )
    for old,new in replacements:
        install=install.replace(old,new)
        verify=verify.replace(old,new)
    wt(root/"Installer/install_gate7_r6f9_triton32_annotation_sanitizer_fix.ps1",install)
    wt(root/"Installer/verify_gate7_r6f9_triton32_annotation_sanitizer_fix.ps1",verify)
    old_install.unlink()
    old_verify.unlink()
...
cd /d "%~dp0"
echo ============================================================
echo 03 - ConceptGhost P10 Gate 7 R6F9
...
echo ============================================================
if not exist "%~dp0Installer\\install_gate7_r6f9_triton32_annotation_sanitizer_fix.ps1" (
  echo [FAIL] Required R6F installer entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\install_gate7_r6f9_triton32_annotation_sanitizer_fix.ps1"
if errorlevel 1 (
...
cd /d "%~dp0"
echo ============================================================
echo 04 - VERIFY ConceptGhost P10 Gate 7 R6F9
echo ============================================================
if not exist "%~dp0Installer\\verify_gate7_r6f9_triton32_annotation_sanitizer_fix.ps1" (
  echo [FAIL] Required R6F verifier entrypoint is missing.
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Installer\\verify_gate7_r6f9_triton32_annotation_sanitizer_fix.ps1"
if errorlevel 1 (
...
def patch_production_workflow(root):
    ...
    workflow=json.loads(rt(path))
    extra=workflow.setdefault("extra",{}).setdefault("conceptghost",{})
    extra["gate7_preview_release"]="GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX"
...
def self_test() -> int:
    ...
    print("CONCEPTGHOST_FLEXGEMM_TRITON32_R6F9_SELFTEST_PASS")
    return 0
    ...
    if cc < (8,0):
        if _minor(torch.__version__) != (2,6) or _minor(triton_version) != (3,2):
            raise RuntimeError(
                ...
                "the private runtime torch 2.6.x + Triton 3.2.x. Re-run the R6F9 installer. "
...
'''
    ...
    lock["release"]="ConceptGhost v1.54 R6F9 MoGe3 Turing Runtime Fix"
    ...
    guide=root/"USER_GUIDE_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX.md"
    wt(guide,rt(guide)+"""
## R6F9 MoGe-3 Turing runtime correction
...
R6F9 applies a bounded compatibility bridge to the ConceptGhost-private FlexGEMM hashmap kernels. FlexGEMM's Python wrappers already serialize keys and queries into padded torch.int32 tensors and pass D as the number of int32 words, so R6F9 removes Triton-JIT dtype-width introspection from those kernels and consumes that existing host-side packing contract directly. Model weights, resolution, refine steps and geometry authority are unchanged.
""")

    validation=root/"P10_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX_VALIDATION.txt"
    wt(validation,rt(validation)+"""
R6F9 MoGe-3 Turing runtime compatibility:
...
""")
    ...
    test=root/"Installer/test_r6f9_triton32_annotation_sanitizer_fix.py"
...
def main(root):
    ...
    for token in ("SANITIZE_NON_CONSTEXPR_TRITON_ANNOTATIONS","sanitize_triton32_annotations","tl.pointer_type","tl.tensor","tl.const","preserved_specialization_annotation","--self-test"):
        if token not in patch: errors.append("R6F9 annotation sanitizer missing "+token)
    try:
        compile(patch, str(patch_path), "exec")
    except SyntaxError as exc:
        errors.append("R6F9 bridge syntax error "+repr(exc))
    probe=subprocess.run([sys.executable,str(patch_path),"--self-test"],capture_output=True,text=True)
    if probe.returncode != 0 or "CONCEPTGHOST_FLEXGEMM_TRITON32_R6F9_SELFTEST_PASS" not in probe.stdout:
        errors.append("R6F9 bridge self-test failed: "+probe.stdout+" "+probe.stderr)
    ...
    print("CONCEPTGHOST_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX_PASS"); return 0
...
def patch_contracts(root,source_commit,package_commit):
    ...
    wt(p,rt(p).replace(
        ...
        "CONCEPTGHOST_P10_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX_RUNTIME_VERIFY_PASS",
    ))

    p=root/"Installer/test_gate7_preview_bundle.py"
    text=rt(p).replace(
        "GATE7_PREVIEW_R5_GEOMETRIC_EVIDENCE_AUDIT_FIX_RUNTIME_VERIFY_PASS",
        "GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX_RUNTIME_VERIFY_PASS",
    ).replace(
        "Installer/install_gate7_preview_r5_geometric_evidence_audit_fix.ps1",
        "Installer/install_gate7_r6f9_triton32_annotation_sanitizer_fix.ps1",
    ).replace(
        "Installer/verify_gate7_preview_r5_geometric_evidence_audit_fix.ps1",
        "Installer/verify_gate7_r6f9_triton32_annotation_sanitizer_fix.ps1",
    )
    wt(p,text)
    ...
    wt(p,rt(p).replace(
        ...
        "USER_GUIDE_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX.md",
    ))
...
"""
    wt(root/"USER_GUIDE_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX.md",guide)
    old_guide.unlink()

    wt(root/"Payload/docs/40_R6F_ROUTE_EDITOR_FINAL.md",f"""# ConceptGhost P10 Gate 7 R6F9 — Route Editor Final
...
""")
    ...
    data.update({
        "schema":"ConceptGhost.P10Gate7R6F9CodeManifest.v0.1",
        "release":R6F,
    ...
    for name in ("RELEASE.json","P10_DR9_RELEASE.json"):
        ...
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F9Release.v0.1",
            "release":R6F,
            "source_commit":source_commit,
    ...
    wt(root/"P10_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX_VALIDATION.txt",f"""ConceptGhost P10 Gate 7 R6F9 Route Editor Final
...
def main(root):
 ...
 print('CONCEPTGHOST_GATE7_R6F9_TRITON32_ANNOTATION_SANITIZER_FIX_PASS'); return 0
...
def rebuild(root,source_commit):
    ...
    for name in ("BUNDLE_MANIFEST.json","P10_DR9_BUNDLE_MANIFEST.json"):
        ...
        data=json.loads(rt(path))
        data.update({
            "schema":"ConceptGhost.P10Gate7R6F9Bundle.v0.1",
            "release":R6F,
            "source_commit":source_commit,
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="cg-r6f-") as temp:
        ...
        tests=(
            ...
            "test_r6f9_triton32_annotation_sanitizer_fix.py",
        )
        for name in tests:

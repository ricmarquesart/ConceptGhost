...
def patch_versions(root: Path) -> None:
    ...
    s = read_text(p)
    replacements = {
        "OPTIONAL MOGE DIAGNOSTICS r9": "OPTIONAL MOGE DIAGNOSTICS r11",
        "ConceptGhost_P10_Lab_before_dr9r_r9": "ConceptGhost_P10_Lab_before_dr9r_r11",
        R9_NAME: R11_NAME,
        "P10_DR9R_R9_READY.json": "P10_DR9R_R11_READY.json",
        "DR9R r9 runtime verification failed.": "DR9R r11 runtime verification failed.",
        "[PASS] DR9R r9 installed and statically verified.": "[PASS] DR9R r11 installed and statically verified.",
    }
    for old, new in replacements.items():
        s = replace_required(s, old, new, f"install_dr9 {old}")
    write_text(p, s)
    # The r11 BAT names are real files, not labels-only substitutions.
    write_text(root / "Installer" / "install_dr11.ps1", s)

    p = root / "Installer" / "verify_dr9.ps1"
    s = read_text(p)
    s = replace_required(s, "P10_DR9R_R9_READY_VERIFY.json", "P10_DR9R_R11_READY_VERIFY.json", "verify report")
    s = replace_required(s, "DR9R r9 installation verified.", "DR9R r11 installation verified.", "verify label")
    write_text(p, s)
    write_text(root / "Installer" / "verify_dr11.ps1", s)
...
def update_bundle_test(root: Path) -> None:
    ...
    s = read_text(p)
    s = replace_required(s, "root/'USER_GUIDE_DR9R_R9.md'", "root/'USER_GUIDE_DR9R_R11.md'", "r11 user guide")
...
def main(root):
    ...
    if release.get("release") != "ConceptGhost_v1.54_P10_DR9R_MOGE_DIAGNOSTICS_TWO_STAGE_INSTALLER_r11":
    ...
    if errors:
        ...
        return 1
    print("CONCEPTGHOST_DR9R_R11_NESTED_INSTALLER_BUNDLE_PASS")
    return 0
...
"""
    ...
    hotfix_doc = f"""# ConceptGhost P10 DR9R — r11 Nested Installer Compatibility Hotfix
...
- r11 hotfix/build commit: `{hotfix_commit or 'LOCAL_BUILD'}`
"""
    write_text(root / "Payload" / "docs" / "23_DR9R_R11_NESTED_INSTALLER_COMPATIBILITY.md", hotfix_doc)

    for name in ("RELEASE.json", "P10_DR9_RELEASE.json"):
        p = root / name
        d = json.loads(read_text(p))
        d["schema"] = "ConceptGhost.P10DR9RRelease.v0.11"
        d["release"] = R11_NAME
        d["status"] = "READY_FOR_USER_RUNTIME_ACCEPTANCE_AFTER_R11_NESTED_INSTALLER_HOTFIX"
    ...
    validation = f"""ConceptGhost P10 DR9R r11 validation
...
def regenerate_manifests(root: Path) -> None:
    ...
    template["schema"] = "ConceptGhost.P10DR9RBundle.v0.11"
    template["release"] = R11_NAME
    template["status"] = "READY_FOR_USER_RUNTIME_ACCEPTANCE_AFTER_R11_NESTED_INSTALLER_HOTFIX"
...
def static_validate(root: Path) -> list[str]:
    ...
    for rel in ("Installer/install_dr11.ps1", "Installer/verify_dr11.ps1"):
        if not (root / rel).is_file():
            errors.append(f"missing r11 user entrypoint target: {rel}")
    bat_install = read_text(root / "03_INSTALL_ALL.bat")
    bat_verify = read_text(root / "04_VERIFY_INSTALL.bat")
    if "Installer\\install_dr11.ps1" not in bat_install:
...
def main() -> int:
    ...
    ap.add_argument("--output", required=True, type=Path, help="r11 ZIP path")
    ...
    with tempfile.TemporaryDirectory(prefix="conceptghost-r11-") as td:
        root = Path(td) / R11_NAME
        root.mkdir()
        ...
        # Imported bundle verifier catches the broader r9 contract with the r11 guide path.
        import subprocess
        for script in ("test_dr9_bundle.py", "test_r11_bundle.py"):
            proc = subprocess.run(
        ...
        print(f"R11_ZIP={args.output}")
        print(f"R11_BYTES={args.output.stat().st_size}")
        print(f"R11_SHA256={digest}")
        if args.keep_extracted:
            if args.keep_extracted.exists():
                shutil.rmtree(args.keep_extracted)
            shutil.copytree(root, args.keep_extracted)
            print(f"R11_EXTRACTED={args.keep_extracted}")
    return 0
